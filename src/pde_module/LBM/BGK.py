...
from pde_module.utils.dummy_types import wp_Array,wp_Vector,wp_Matrix,wp_Dtype
...
class BGK_collision(LBM_Stencil):
    '''
    Standalone Module for BGK Collision Operator
    '''
    ...
    def initialise(self,f_in,tau):
        ...
        self.kernel = create_BGK_collision(self.latticeModel.float_directions,self.latticeModel.weights,self.num_distributions,self.dimension,self.grid_shape,self.latticeModel.float_dtype)
...
def create_BGK_collision(float_velocity_directions,weights,num_distributions,dimension,grid_shape,float_dtype):
