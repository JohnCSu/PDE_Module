...
def create_grad_kernel(dimension,levels,num_outputs):
    ...
    def grad_kernel(
        ...
        for axis in range(wp.static(D)):
            # print(adj_points)
            adj_values = get_adjacent_values(current_values,batch_id,x_id,y_id,z_id,axis)
            new_values[batch_id,x_id,y_id,z_id][axis] = alpha*first_order.central_difference(adj_values[1],current_value,adj_values[0],dx)[0]
...
def create_Divergence_kernel(dimension,levels,num_inputs,div_type):
    ...
    def divergence_kernel(
        ...
        for axis in range(wp.static(D)):
            ...
            div_val += alpha*first_order.central_difference(adj_values[1][axis],current_value[axis],adj_values[0][axis],dx)
...
def create_tensor_divergence_kernel(dimension,levels,mat_shape,div_type):
    ...
    def divergence_kernel(
        ...
        for axis in range(wp.static(D)):
            ...
            div_val += alpha*first_order.central_difference(adj_values[1],current_value[:,axis],adj_values[0],dx)
